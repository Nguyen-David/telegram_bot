...
class Screen:
    ...
    def get_first_screen(self, mass, next_step, back, msg_text):
        ...
        msg = self.bot.send_message(self.message.chat.id, dialog[msg_text], reply_markup=markup, parse_mode="Markdown", disable_web_page_preview='true')
        self.bot.register_next_step_handler(msg, next_step)

    def get_current_screen(self, mass, next_step, back):
        markup = create_menu(mass, back=back)
        msg = self.bot.send_message(self.message.chat.id, dialog[self.message.text], reply_markup=markup, parse_mode="Markdown", disable_web_page_preview='true')
        self.bot.register_next_step_handler(msg, next_step)
    ...
    def get_recursive_screen(self, current_step):
        msg = self.bot.send_message(self.message.chat.id, dialog[self.message.text], parse_mode="Markdown", disable_web_page_preview='true')
        self.bot.register_next_step_handler(msg, current_step)

    def get_error_screen(self, current_step):
        msg = self.bot.send_message(self.message.chat.id, 'Простите я вас не понял(', parse_mode="Markdown", disable_web_page_preview='true')
        self.bot.register_next_step_handler(msg, current_step)
    ...
    def get_popular_question_screen(self, mass, company, next_step):
        ...
        msg = self.bot.send_message(self.message.chat.id, "*НАЙПОПУЛЯРНІШІ ПИТАННЯ *" + "*" + self.message.text + "*", reply_markup=markup, parse_mode="Markdown", disable_web_page_preview='true')
        self.bot.register_next_step_handler(msg, next_step)
    ...
    def get_popular_question_edit_screen(self,  msg_text, back):
        ...
        self.bot.send_message(self.message.chat.id, dialog['ПОВЕРНУТИСЯ ДО ПИТАНЬ'], reply_markup=markup, parse_mode="Markdown", disable_web_page_preview='true')